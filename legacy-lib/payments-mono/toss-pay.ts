...
export class TossPayService implements PaymentSerivce {
   ...
   * @returns
   */
  static getInstance(autoExecute: boolean = false, apiKey?: string) {
    if (this._instance === undefined) {
      this._instance = TossPayService.BUILD(autoExecute, apiKey);
    }
    return this._instance;
  }
  static BUILD(autoExecute: boolean = false, apiKey?: string): TossPayService {
    const { tosspay } = getSecret();
   ...
   * @returns
   */
  async approveOnetime(payToken: string) {
    const requestParam = {
      apiKey: this.apiKey,
      payToken: payToken,
    };
...
export interface TossPayCancelPaymentParam extends PaymentParam {
  payToken: string;
  amount: number;
  amountTaxable: number;
  amountTaxFree: number;
